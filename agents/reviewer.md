...
2. 同步回 plan 阶段跟踪表：阶段状态改为 `⚠️ review 打回`，备注 `review-{功能名}-v{版本号} #问题编号`
3. 修复完成后双向更新：review 遗留问题 `⬜ → ✅ 已修复 YYYY-MM-DD`，plan 阶段状态 `⚠️ → ✅ 已完成（review 后修复）`
...
签收通过后，将有价值的经验写入 `MEMORY.md`：踩坑及解决方案、非直觉行为或隐含约束、编译/构建/依赖等环境问题。

## 代码审查（按需）

当需要代码审查时，使用 `reviews/template/code_review_template.md`，审查维度：命名规范、项目结构、编码规范、注释规范、UI 布局、ObjC/Swift 混编。
